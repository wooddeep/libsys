...
func (this *ExperienceController) Lst() {
	const tabelName = "experience"
	...
	var list []models.Experience //查询的结果是集合的话，这里需要加上[]
...
func (this *ExperienceController) Add() {
	data := this.Ctx.Input.RequestBody
	var object models.Experience
...
func (this *ExperienceController) Del() {
	data := this.Ctx.Input.RequestBody
	var object models.Experience
	err := json.Unmarshal(data, &object)
	if err != nil {
...
func (this *ExperienceController) Mod() {
	const tabelName = "experience"
	data := this.Ctx.Input.RequestBody
	var object models.Experience
	err := json.Unmarshal(data, &object)
...
func init() {
	...
	const rootPath = "experience"
	beego.Router(fmt.Sprintf("/%s/lst/:pi/:ps", rootPath), &ExperienceController{}, "get:Lst")
	beego.Router(fmt.Sprintf("/%s/add", rootPath), &ExperienceController{}, "post:Add")
	beego.Router(fmt.Sprintf("/%s/del", rootPath), &ExperienceController{}, "delete:Del")
	beego.Router(fmt.Sprintf("/%s/mod", rootPath), &ExperienceController{}, "put:Mod")
}
