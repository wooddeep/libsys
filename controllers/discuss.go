...
func (this *DiscussController) Lst() {
	...
	var list []models.Discuss //查询的结果是集合的话，这里需要加上[]
	qs := o.QueryTable("discuss")
...
func (this *DiscussController) Add() {
	data := this.Ctx.Input.RequestBody
	var object models.Discuss
...
func (this *DiscussController) Del() {
	data := this.Ctx.Input.RequestBody
	var object models.Discuss
	err := json.Unmarshal(data, &object)
	if err != nil {
...
func (this *DiscussController) Mod() {
	data := this.Ctx.Input.RequestBody
	var object models.Discuss
	err := json.Unmarshal(data, &object)
	if err != nil {
	...
	n, err := o.QueryTable("discuss").Filter("id", object.ID).Update(mapResult)
	if err == nil {
		var data = map[string]interface{}{"num": n}
		this.Data["json"] = map[string]interface{}{"data": data, "msg": "success", "code": 0} // 设置返回值
		this.ServeJSON()
...
func init() {
	var rootPath = "discuss"
	beego.Router(fmt.Sprintf("/%s/lst/:pi/:ps", rootPath), &DiscussController{}, "get:Lst")
	beego.Router(fmt.Sprintf("/%s/add", rootPath), &DiscussController{}, "post:Add")
	beego.Router(fmt.Sprintf("/%s/del", rootPath), &DiscussController{}, "delete:Del")
	beego.Router(fmt.Sprintf("/%s/mod", rootPath), &DiscussController{}, "put:Mod")
}
