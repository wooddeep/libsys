...
const tabelName = "circle"
const rootPath = "circle"

func (this *CircleController) Lst() {
	...
	var list []models.Circle //查询的结果是集合的话，这里需要加上[]
...
func (this *CircleController) Add() {
	data := this.Ctx.Input.RequestBody
	var object models.Circle
...
func (this *CircleController) Del() {
	data := this.Ctx.Input.RequestBody
	var object models.Circle
	err := json.Unmarshal(data, &object)
	if err != nil {
...
func (this *CircleController) Mod() {
	data := this.Ctx.Input.RequestBody
	var object models.Circle
	err := json.Unmarshal(data, &object)
	if err != nil {
...
func init() {
	beego.Router(fmt.Sprintf("/%s/lst/:pi/:ps", rootPath), &CircleController{}, "get:Lst")
	beego.Router(fmt.Sprintf("/%s/add", rootPath), &CircleController{}, "post:Add")
	beego.Router(fmt.Sprintf("/%s/del", rootPath), &CircleController{}, "delete:Del")
	beego.Router(fmt.Sprintf("/%s/mod", rootPath), &CircleController{}, "put:Mod")
}
